use crate::json_loader::entities::{JsonStructure, Attribute};
...
impl DDLProvider {
    ...
    pub fn generate_create_table(&self, json_structure: JsonStructure) -> String {
        ...
        for (idx, table_content) in tables.iter() {
            let tablename: String = idx.to_owned(); 

            let table_query = self.table_json_to_sql(tablename, &table_content.attributes).unwrap_or_else(|error| {
                panic!("Problem: {:?}", error)
            });
    ...
    fn table_json_to_sql(&self, tablename: String, attributes: &Vec<Attribute>) -> Result<String, InvalidTableDefinition> {
        if tablename.len() < 1 {
